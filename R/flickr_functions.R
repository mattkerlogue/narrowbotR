...
canal_word_count <- function(string) {
  ...
  for (word in canal_words) {
    x <- str_detect(tolower(string), word)
    counter <- counter + x
  }
...
flickr_photo_score <- function(df) {
  ...
  n_df <- df %>%
    select(id, owner, title, description, date, goldenhour) %>%
    mutate(
      title_words = map_dbl(title, ~max(str_count(., " "),1)),
      desc_words = str_count(description, " "),
      total_words = title_words + desc_words,
      desc_words2 = log10(map_dbl(description, ~max(str_count(., " "),1))),
      word_score = pmap_dbl(list(title_words, desc_words2), sum, na.rm = TRUE),
      canal_words = canal_word_count(title) + canal_word_count(description),
      canal_word_score = map_dbl(canal_words^2, ~max(., 1)),
      gold = if_else(goldenhour, 2, 1),
...
flickr_pick_photo <- function(df) {
  ...
  n_df <- df %>% 
    inner_join(scored_df, by = c("id", "owner")) %>%
    drop_na(final_score) %>%
